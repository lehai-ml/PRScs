...
Essentially, if the string "rs" is not present in SNP columns of the bim or summary stat filess. The code will match the summary stat, bim file and the ld reference panel with "CHR:BP" format instead. So reference panel will look at "CHR:BP" instead of "SNP rsid".

This step will run for a while.

5) Concatenate all output files into one single file.
6) Calculate the PLINK score.
